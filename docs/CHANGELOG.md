...
- The economy rework (99997, 999958, 27 WorkPlaceEvents dispatchers) gated on `tag = BHU`, so annexing Bhutan switched it off. Now gated on the `economy_pulse_host` country flag, elected by new event 2000100 and re-elected by watchdog 2000101.
...
- `on_actions` weights are picks, not chances — the engine fires exactly one event per country per pulse. The 2000101 watchdog had been added to both the yearly and quarterly lists, silently cutting the `Canals.txt` 97200/97250/97350 rate; the yearly entry is gone.
