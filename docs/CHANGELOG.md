...
- The triggered modifier `money_hoarder_1..4` (and 121 other definition names) displayed its raw script identifier in game because no csv defined it. New `scripts/refcheck.py defs` check lists every player-visible definition name without a localisation key (modifiers, national focus, issue options, ideologies, governments, goods, buildings, factory production types, rebel types, casus belli, crime, cultures, religions, pop types, tags, technologies, inventions), each convention read off the vanilla files. All 125 findings filled in `localisation/00_CoE_RoI_definitions.csv`; the check is now part of the default run and of the `/validate` baseline at 0.

### Religion restoration — **UNTESTED IN GAME**

Owner decision to undo the transposition that had put the pop's culture name in the religion
field and commented the real religion out. Plan: `docs/design/religion-restoration.md`;
adversarial review: `docs/design/religion-restoration-review.md`; post-state:
`docs/audit/religion-dead-content.md`. Five commits, one per step, so a startup failure bisects.

- **Pops** (`scripts/religion_restore.py --apply`): 22 936 lines in 61 of 62 `history/pops/1821.9.1` files. The religion field goes back to the real religion (21 024 recovered from the `#religion =` comment, 2 from git `a3a51224`), and 1 910 pops move their sub-culture into the `culture` field — `north_german` 604, `south_german` 534, `dixie` 256, `north_italian` 206, `south_italian` 94, `anglo_canadian` 68, `occitan` 58, `picard` 42, `australian` 30, `anglo_african` 18. 16 religions are now carried by pops; `german` and `italian` have **0 pops**.
- **`history/countries`**: 53 German/Italian tags take their sub-culture as `primary_culture`, read off their own `<subculture>_rel` flag. Accepted cultures mirror vanilla per the review: PRU/GER/NGF +south_german, SGF +north_german, ITA +south_italian; PAP/SWI/DLM replace the now-dead `culture = italian` with `north_italian` (DLM was invisible to the flag method — its primary is `croat`); USA +dixie, CSA flips to `dixie` primary + `yankee` accepted, CAN to `anglo_canadian` primary, OCC to `occitan` primary + `french`, AST +australian, SAF +anglo_african, FRA +occitan +picard, ENG +anglo_canadian only. AUS/BAV/KUK deliberately stay `south_german` alone, because the Austro-Prussian dualism content depends on Austria not being a north-German state.
- **Triggers**: all 121 live culture-named religion sites in 29 files converted 1:1 — `has_pop_religion` -> `has_pop_culture` (90), pop-scope `religion` inside an `any_pop` limit -> `has_pop_culture` (19), province-scope `religion` inside `any_owned`/`any_core`/`all_core` -> `culture` (12). This includes the 6 `french`/`breton`/`wallonian` sites in `events/+education_RGO_b.txt` and `events/AUSFlavor.txt:1268` that the design's table had missed, and all **7** negated sites (not 3): leaving one converted would have made an `allow` gate universally true — `outside_form_italy` would have annexed sphered Italian states with zero territory held, `form_north_german_confederation2` likewise, and `Alternative ACW` would have stripped CSA cores from nearly every state.
- **The 32 `german`/`italian` culture sites**: 18 `add_accepted_culture` effects now grant both sub-cultures (matching what vanilla does after `change_tag = GER`/`ITA`, since `change_tag` does not re-apply the target's history cultures); the `primary_culture = german|italian` triggers become `is_culture_group = germanic|italian`; AUS's `end_danube_federation` sets `south_german`; the `has_pop_culture` and province-`culture` tests list both sub-cultures.
- **`common/religion.txt`** truncated after the `pagan` group: 3 271 -> 148 lines, 43 -> 6 groups, 293 -> 24 religions. `common/cultures.txt` needed nothing — all 10 sub-cultures already existed in the right groups, and the GER/ITA/USA union tags hang off the *group*, so union mechanics, `change_tag = culture` and the pan-nationalist content are untouched.
- Reinstated the three 2026-09-06 dead-negative deletions now that they mean something again: the `carlist_rebels` `NOT = { has_pop_religion = catholic }` spawn modifier and the two `events/Dungan.txt` sunni limits.
- **New regression gate** `python scripts/audit_religion.py check` (added to the `/validate` baseline at 0 problems): asserts no pop holds a culture name as a religion, no trigger names a culture as a religion, no `culture = german|italian` site exists, and `common/religion.txt` defines no culture as a religion.

Risks, none of them observed in a running game:

- **~124 religion triggers went live at once.** Nothing gates on a real religion in a `trigger`/`potential`/`allow` block — every site is a `limit` inside an effect — so nothing new fires on day one, but the Persian Sunni/Shiite chain (`events/DIM/PERFlavour_five_x.txt`, 155 sites, earliest self-firer 1832), the Dungan massacre limits, MOR 290100 and the Taiping/Sepoy/Brazil/England chains now do what they say. All need a balance review as a separate pass.
- **Religious-policy pop weights rebase mod-wide.** The 48 `is_state_religion` sites in `poptypes/` (4 in each of 12 files) were previously either universally true or universally false: `secularized`'s 1.50x now applies only to religious minorities instead of every pop, and the `moralism` / `pluralism` 1.25x blocks stop being no-ops. `common/pop_types.txt:88`'s -0.1 militancy under moralism narrows the same way. Deliberately **not** retuned in this pass.
- **Accepted-culture changes are player-visible on day one.** Every German and Italian state now accepts its own sub-culture only; foreign-held provinces (Austrian Lombardy, Danish Holstein, French Alsace) are where separatist/nationalist behaviour genuinely shifts, and `nationalist_rebels` uses `independence = culture`, so which of the 26 `north_german`-primary tags gets released there is engine-chosen. Canada, Australia and the Cape become non-accepted under ENG (vanilla behaviour, but new here); France accepting `occitan` and `picard` is a mod-only choice made to avoid an unhistorical 1821 Midi/Picardy nationalism spike.
- **Assimilation across a religion boundary becomes active for the first time** (the mod defines no `assimilation_chance`, so the engine default applies). Habsburg lands and the Ottoman Balkans are the visible cases. Conversion stays hard-disabled.
- Smoke test to run: the game reaches the map at all (a bad `religion.txt` truncation is a startup crash); PRU/BAV/AUS/SAR/SIC/USA/ENG/FRA show sane accepted-culture and militancy numbers on day 1; the population screen's religion pie is a plausible mix; a year on observer with an `error.log` diff.
...
- Script made coherent with the new `defines.lua` values (follow-up to the four restored defines above). **Infamy grants**: every effect-scope `badboy = N` above 10 halved so it keeps the share of the limit it had at 50 - 38 lines in 17 files, headed by the 1848 German revolution options (150 -> 75, 125 -> 63, 100 -> 50, 80 -> 40, 75 -> 38, 50 -> 25, 40 -> 20, 20 -> 10), `Greater Germany.txt` (40 -> 20, 20 -> 10), `decisions/France.txt` `fra_setup` and `Ottoman_Dec.txt` `ioanninia_dynasty` (40 -> 20), and the `BYZ_Expansion` / `ACW` / `GRE` / `AUS` / `SWI_neutrality` / `PORFlavor` 25-12 tier. Grants of 10 and below, the `badboy = -1000` wipes and the `badboy = 24.99` clamps beside them are unchanged (that pair was written for a limit of 25 and is correct again). **Infamy triggers**: the `badboy` *trigger* is a fraction of `BADBOY_LIMIT`, not an absolute - confirmed from `docs/wiki/list-of-conditions.md:317` and from vanilla, where every effect-scope value is an integer 1-10 and every trigger-scope value a fraction 0.2-0.8 - so the twelve integer thresholds in `decisions/BYZ_Expansion.txt` (10 `ai_will_do` guards) and `events/RUSFlavor.txt:1897, :3181` demanded 125-500 infamy and never fired; converted to 0.2 / 0.4 / 0.6 / 0.8. The `InfamyWar_Events.txt` and `crises.txt` containment thresholds (1 / 1.5 / 2) were already fractions and are left alone: they now trip at 25 / 37.5 / 50 infamy, which is the point of the change. **Administrative efficiency**: `BASE_COUNTRY_ADMIN_EFFICIENCY` 1.0 -> **0.2** (vanilla), so efficiency is earned through bureaucrats and administrative reforms instead of being pinned at 100% - without this the restored `MAX_BUREAUCRACY_PERCENTAGE` / `BUREAUCRACY_PERCENTAGE_INCREMENT` did nothing. Tax revenue for an unadministered country drops sharply (it multiplies the mod's `BASE_COUNTRY_TAX_EFFICIENCY` 0.50) and bureaucrat demand rises by an order of magnitude; the `admin_found_*` ladder in `common/triggered_modifiers.txt` was re-cut from 0.005-0.050 to 0.003-0.030 in 0.003 steps so all ten tiers sit inside the reachable 1.0-2.9% band. Full reasoning and the left-alone lists are in `docs/audit/common.md`, "Follow-ups after the defines change".
