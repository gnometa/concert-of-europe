...
- `loc-check` column checks (added 2026-09-06): the command now also reports (a) rows where a `;` typed inside the English text splits it across the English and French columns, and (b) a per-file count of rows that are not exactly 15 columns wide. Baseline over `CoE_RoI_R/localisation/*.csv`: **14 split rows** (all vanilla/PDM-inherited: `00_PDM_events.csv` 2, `00_PDM_GAGA.csv` 2, `00_PDM_goods.csv` 1, `000_persia_events.csv` 1, `000_persia_map.csv` 1, `PDM_CE.csv` 3, `Taiping(move_later).csv` 1, `text.csv` 2, and `GVG_events.csv:16` `EVTDESC999999` from the education/RGO rework) and **27904 odd-width rows across 48 files** (harmless trailing/missing empty language columns, dominated by `text.csv`). New GVG content must add none of either.
