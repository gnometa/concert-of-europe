...
| `audit_owner_scope.py` | 0 highs, 0 mediums, 147 lows | advisory. Every low is a conditional release/secede or cede branch where the scope may legitimately own the province later in the game; a **high** means the block is dead in every reachable state and must be rescoped (`TAG = { any_owned = ... }`) |
