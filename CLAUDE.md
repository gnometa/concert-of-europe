...
- Pops carry no real religion: the pop religion field holds the culture (or sub-culture) name and the true religion is commented out, so religion-based triggers/effects (`has_pop_religion = orthodox`, `pop_majority_religion = catholic`, religion-policy content) match nothing; test cultures instead.
