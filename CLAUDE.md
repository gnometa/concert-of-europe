...
- **A key the parser does not accept in that position eats a brace and silently reshapes the file.** The engine reads the unexpected `{` (or the unexpected scalar) as the value and carries on, so the block's own `}` closes its *parent*: every sibling after it is promoted one level, a decision's `effect`/`ai_will_do` become top-level decisions visible to every country as cards reading `effect_title`, and event files just lose events. Braces balance, `error.log` stays empty, CWTools says nothing. Three shapes, all found 2026-09-06: an effect-only iterator used as a trigger (`any_country`, `any_owned`, `random_country`, `random_owned`, `random_pop`, `random_state` — `any_pop`, `any_state`, `any_substate`, `any_sphere_member`, `any_greater_power`, `any_owned_province`, `war_countries`, `capital_scope` and bare tag/province scopes are all fine); `province_event = <id>` without the block form; and a scalar effect handed a block (`set_country_flag = { x }`). `python scripts/modcheck.py desync` catches all three statically. The mechanism-agnostic check is `python scripts/modcheck.py engine-counts`, which diffs the `Decisions loaded … #N` / `Events loaded … #N` counts the engine writes to `setup.log` against a local parse — run it after any `gametest.ps1` launch.
...
- **A mod file only replaces the vanilla file at the exact same relative path.** Rename it or omit it and vanilla's copy loads *alongside* the mod's: `events/` and `decisions/` are additive, so the ids/names are defined twice and one silently wins; a `localisation/*.csv` the mod does ship replaces the vanilla file wholesale, so keys the mod's copy omits are simply gone. Nothing is logged either way — `setup.log` just lists both files. Vanilla ships two Socialism event files (`Socialism_Facism.txt` with the content, `Socialism_Fascism.txt` empty), which is how the mod's correctly-spelled copy shadowed the wrong one for years. `python scripts/modcheck.py shadow` enforces it.
...
- **PostToolUse** runs brace/CRLF/encoding checks on every edited `.txt` under `CoE_RoI_R/`, plus province-id, tag and parser-desync checks for events, decisions, and history files. Fix what it reports before moving on.
